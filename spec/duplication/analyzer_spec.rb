...
RSpec.describe Hashira::Duplication::Analyzer do
  it "reports one finding per cluster, with each site as evidence and a refactoring" do
    analyze_duplication(FixtureHelper::DUPLICATION_FILES) do |analyzer|
      ...
      expect(finding.evidence).to include(a_string_matching(%r{orders/checkout\.rb:1-11}),
  ...
  it "finds no duplication in code that has none" do
    ...
    analyze_duplication(files) { |analyzer| expect(analyzer.findings).to be_empty }
  end
