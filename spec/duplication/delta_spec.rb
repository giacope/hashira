...
RSpec.describe Hashira::Duplication::Delta do
  def kind_for(sources) = described_class.new(cluster_for(sources)).kind
  ...
  it "reports :identical when the sites are byte-for-byte the same" do
    expect(kind_for(clone("g.emit(fetch(:h), fetch(:p))", "g.emit(fetch(:h), fetch(:p))"))).to eq(:identical)
  end

  it "reports :literal when only literal values differ" do
    expect(kind_for(clone("emit(fetch(:h), 1)", "emit(fetch(:h), 9)"))).to eq(:literal)
  end

  it "reports :message when only the receiver differs" do
    expect(kind_for(clone("x.emit(fetch(:h), fetch(:p))", "y.emit(fetch(:h), fetch(:p))"))).to eq(:message)
  end

  it "reports :constant when only a constant reference differs" do
    expect(kind_for(clone("Foo.emit(fetch(:h), fetch(:p))", "Bar.emit(fetch(:h), fetch(:p))"))).to eq(:constant)
  end

  it "reports :mixed when more than one kind of thing differs" do
    expect(kind_for(clone("x.emit(fetch(:h), 1)", "y.emit(fetch(:h), 9)"))).to eq(:mixed)
  end
  ...
  it "reports :structure when the control flow differs across a near-miss" do
    ...
    expect(kind_for(near)).to eq(:structure)
  end

  it "serializes to a hash and maps the kind to a refactoring summary" do
    delta = described_class.new(cluster_for(clone("emit(fetch(:h), 1)", "emit(fetch(:h), 9)")))
