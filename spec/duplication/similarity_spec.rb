...
RSpec.describe Hashira::Duplication::Similarity do
  ...
  it "accepts a pair that clears the threshold" do
    expect(described_class.new(%i[a b c d], %i[a b x d]).at_least?(0.75)).to be(true)
  end

  it "rejects a pair whose token mix cannot reach the threshold" do
    expect(described_class.new(%i[a b c d], %i[a x y z]).at_least?(0.8)).to be(false)
  end
  ...
  it "still runs the full comparison when the bound alone would pass" do
    ...
    expect(described_class.new(%i[a b c d], %i[d c b a]).at_least?(0.8)).to be(false)
  end

  it "counts a repeated token only as often as both sides carry it" do
    expect(described_class.new(%i[a a a a], %i[a a b b]).at_least?(0.6)).to be(false)
  end
end
