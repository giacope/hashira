...
RSpec.describe Hashira::Hotspots::Rollup do
  ...
  def cluster(mass, *files)
    instance_double(Hashira::Duplication::Cluster, site_masses: files.map { [it, mass] })
  end
  ...
  it "zeroes the column of a skipped analyzer rather than breaking" do
    only_dupes = described_class.new(nil, duplication([cluster(9, "a.rb")]), Hashira::Churn.new({}))

    expect(only_dupes.files.map { [it.file, it.cognitive, it.duplication] }).to eq([["a.rb", 0, 9]])
  end
end
